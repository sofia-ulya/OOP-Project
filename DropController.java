...
public class DropController implements Initializable {
    ...
    private void handleFormSubmission(ActionEvent event) throws IOException {
        ...
        switch (Semester.Sem.s.getYearSemester()) {
            case 1:
                if(credit<=0){
                    totalCredit.setText("Total Credit: Error credit exceeds minimum");
                    JOptionPane.showMessageDialog(null, "Credit exceeds minimum.");}
                else {
                    FXMLLoader loader = new FXMLLoader();
                    loader.setLocation(getClass().getResource("ConfirmDrop.fxml"));
                    Parent menuBtn = loader.load();
                    System.out.println("Confirmation to drop subject(s)!");
        
                    Scene scene = new Scene(menuBtn);
                    Stage nextWindow = (Stage) ((Node)event.getSource()).getScene().getWindow();
        
                    ConfirmDropController confirm = loader.getController();
        
                    nextWindow.setTitle("Confirm Drop Subject");  //set title utk pop up
